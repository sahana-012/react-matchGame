...
class ImageStore extends Component {
  ...
  changeCounter = id => {
    ...
    if (randomImage.id === id) {
      this.setState(prevState => ({
        ...
        randomImage: imagesList[Math.ceil(Math.random() * imagesList.length)],
      }))
    } else if (timer === 0) {
      this.clearTimer()
    } else {
  ...
  renderGame = () => {
    const {activeId, randomImage} = this.state
  ...
  renderWinOrLose = () => {
    ...
    return (
      <WinOrLose timer={timer} score={score} replayGame={this.replayGame} />
    )
  }
